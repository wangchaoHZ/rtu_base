...
static int mtr_rtu_build_request_header(mtr_rtu_mb_t *smb, uint8_t *buff, int slave, int fun, int reg, int num)
{
    buff[0] = slave;
    buff[1] = fun;
    buff[2] = reg >> 8;
...
static int mtr_rtu_build_response_header(mtr_rtu_mb_t *smb, uint8_t *buff, int slave, int fun)
{
    buff[0] = slave;
    buff[1] = fun;
...
static int mtr_rtu_check_send_pre(mtr_rtu_mb_t *smb, uint8_t *buff, int length)
{
...
static int mtr_rtu_check_wait_request(mtr_rtu_mb_t *smb, uint8_t *buff, int length)
{
    uint16_t crc_recv =
        ((uint16_t) (buff[length - 2] & 0x00FF) + (uint16_t) (buff[length - 1] << 8)); // LOW BYTE HIGH BYTE
...
static int mtr_rtu_check_wait_response(mtr_rtu_mb_t *smb, uint8_t *buff, int length)
{
    uint16_t crc_recv =
        ((uint16_t) (buff[length - 2] & 0x00FF) + (uint16_t) (buff[length - 1] << 8)); // LOW BYTE HIGH BYTE
    uint16_t crc_cal = mtr_rtu_mb_crc16(buff, length - 2);
...
const mtr_rtu_mb_core_t _mtr_rtu_mb_core = {.magic = MODBUS_CORE_MAGIC,
