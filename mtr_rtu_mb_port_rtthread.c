...
static int _mtr_rtu_mb_rtdevice_open(mtr_rtu_mb_t *smb) {
    mtr_rtu_mb_port_rtdevice_t *smb_port_device =
            (mtr_rtu_mb_port_rtdevice_t *) smb->port;
    if (smb_port_device->device) {
        ...
        rt_device_set_rx_indicate(smb_port_device->device,
                                  _mtr_rtu_mb_rtdevice_rx_indicate);
...
static int _mtr_rtu_mb_rtdevice_write(mtr_rtu_mb_t *smb, uint8_t *data,
                               uint16_t length) {
    mtr_rtu_mb_port_rtdevice_t *smb_port_device =
            (mtr_rtu_mb_port_rtdevice_t *) smb->port;
...
static int _mtr_rtu_mb_rtdevice_read(mtr_rtu_mb_t *smb, uint8_t *data,
                              uint16_t length) {
    mtr_rtu_mb_port_rtdevice_t *smb_port_device =
            (mtr_rtu_mb_port_rtdevice_t *) smb->port;
...
mtr_rtu_mb_port_rtdevice_create(const char *device_name) {
    mtr_rtu_mb_port_rtdevice_t *smb_port_device =
            rt_malloc_align(sizeof(mtr_rtu_mb_port_rtdevice_t), 4);
    if (smb_port_device) {
...
int mtr_rtu_mb_rtu_set_serial_rts(mtr_rtu_mb_t *smb, int (*rts_set)(int on)) {
    mtr_rtu_mb_port_rtdevice_t *smb_port_device =
            mtr_rtu_mb_port_rtdevice_get(smb);
    if (smb_port_device) {
