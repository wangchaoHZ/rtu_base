 ...
 * @FilePath: \STD_BMS_MB_FW\MB_RTU_MTR\rtu1_mb_master_app.c
...
static rtu1_mb_t rtu1_master = {0};

static rt_bool_t rtu1_master_startup = RT_FALSE;
static rt_mutex_t rtu1_master_mux = RT_NULL;

#define TESTING_FUNCTION_CODE 0x10

// RTU主站回调函数
void rtu1_master_poll(rtu1_mb_t* rtu_master)
{
    int rc = 0, index = 0;
    ...
    // 设置从站地：1
    rtu1_mb_set_slave(rtu_master, 1);
    ...
    for (;;)
    {
        ...
        if (rtu1_master_startup)
        {
            rc = rtu1_mb_read_bits(rtu_master, 0, 8, temp_buff);  // modbus_read_input_bits
            LOG_I("Master Read 01 Coil Address 0x0000 Length 8: %d", rc);
            if (rc >= MODBUS_OK)
            {
                for (index = 0; index < 8; index++)
                {
                    rt_kprintf("[%d]", rtu1_dio_get_val(temp_buff, index));
                }
                rt_kprintf("\n");
            }
        }
        ...
        if (rtu1_master_startup)
        {
            rc = rtu1_mb_read_input_bits(rtu_master, 0, 8, temp_buff);  // modbus_read_bits
        ...
        if (rtu1_master_startup)
        {
            rc = rtu1_mb_read_registers(rtu_master, 0, 8, temp_buff);
            LOG_I("Master Read 03 Register Address 0x0000 Length 8: %d", rc);
            if (rc >= MODBUS_OK)
            {
                for (index = 0; index < 8; index++)
                {
                    rt_kprintf("[%d]", rtu1_aio_get_val(temp_buff, index));
                }
                rt_kprintf("\n");
            }
        }
        ...
        if (rtu1_master_startup)
        {
            rc = rtu1_mb_read_input_registers(rtu_master, 0, 8, temp_buff);
        ...
        if (rtu1_master_startup)
        {
            rc = rtu1_mb_write_bit(rtu_master, 0, coils_state);
        ...
        if (rtu1_master_startup)
        {
            rc = rtu1_mb_write_register(rtu_master, 0, regs_value);
        ...
        if (rtu1_master_startup)
        {
            rc = rtu1_mb_write_registers(rtu_master, 0, 8, write_regs);
...
// RTU主站线程入口函数
static void rtu1_master_thread_entry(void* params)
{
    // 获取RTU主站实例
    rtu1_mb_t* app_master = params;

    // 创建互斥量
    rtu1_master_mux = rt_mutex_create("mtr_mux", RT_IPC_FLAG_PRIO);

    // 配置串口参数
    struct serial_configure serial_config = serial_params_config();
    rtu1_mb_rtu_set_serial_config(app_master, &serial_config);

    // 初始化串口设备
    rtu1_mb_init(app_master, MODBUS_CORE_RTU, rtu1_mb_port_rtdevice_create(RTU1_UART_DEVICE_NAME));

    // 设置串口RTS和读写标志
    rtu1_mb_rtu_set_oflag(app_master, RT_DEVICE_FLAG_RDWR | RT_DEVICE_FLAG_DMA_RX);

    // 建立RTU连接
    rtu1_mb_connect(app_master);

    // 设备读写管理
    rtu1_master_poll(app_master);
}

// RTU主站线程初始化函数
int rtu1_master_thread(void)
{
    // 创建RTU主站线程
    rt_thread_t thread_id = rt_thread_create("mtr-master",                 // 线程名称
                                             rtu1_master_thread_entry,  // 线程入口函数
                                             &rtu1_master,              // 线程入口参数
                                             1024,                         // 线程栈大小
                                             13,                           // 线程优先级
                                             20                            // 时间片
...
MSH_CMD_EXPORT(rtu1_master_thread, "RTU Master thread for MTR Modbus");
